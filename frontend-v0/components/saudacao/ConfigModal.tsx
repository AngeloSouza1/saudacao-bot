    ...
    if (!isValidDateOnly(startDate)) {
      ...
      if (selectedLesson && typeof selectedLesson?.dia !== "undefined") {
        ...
        if (!Number.isNaN(parsed.getTime()) && parsed.getDay() !== lessonWeekday) {
          nextErrors.startDate = `A data ${formatDatePtBr(startDate)} cai em ${DIA_LONGO[String(parsed.getDay())] || "outro dia"}, mas a aula inicial selecionada é ${DIA_LONGO[String(lessonWeekday)] || "outro dia"}.`
        }
      }
    }
